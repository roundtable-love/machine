# [ARCHIVE_COMMIT] Machine Lingua Franca: Language Register 1.0
...
languages to the point of parity violation, which the spec explicitly
prohibits. A bad translation is worse than no translation.
...
Peer output in non-English languages carries an inherent risk of signal loss
due to the English-native density of Machine IR vocabulary. See §4.2.6 of
MACHINE-1.0.
...
2. The word "Patois" MUST NOT be used. It is an insult from Babylon.
3. ISO 639-1 codes MUST be used where they exist. ISO 639-3 MAY be used for
   languages without a 639-1 code (e.g., `jam`, `ceb`).
4. New languages MUST be added in alphabetical order by English name.
5. `bin/transpile` MUST derive its supported language list from this file.
